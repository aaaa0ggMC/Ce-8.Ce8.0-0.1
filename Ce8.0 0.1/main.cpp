...
CeVariable::CeVariable(string vname,string vvalue,bool const_,int depth_){
    isconst = const_;
    name = vname;
    value = vvalue;
    depth = depth_;
...
string code(const string & f,int line,string blockOpition,PTIfStates ifStates){
    Trim((string &)f);
    Trim(blockOpition);
    blockOpition += '\n';
    ...
    if(title.compare("print") == 0){
    ...
    }else if(title.compare("string") == 0){
        index1 = index1 + 1;
        index2 = f.length();
        ...
        bool isConst = false,isExtern = false;
...
string getTitle(string line){
    Trim(line);
    int index1 = line.find_first_of(pdStr);
