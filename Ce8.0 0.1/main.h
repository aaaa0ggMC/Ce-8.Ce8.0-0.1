...
public:
    ...
    *Return :无
    */
    CeVariable(string vname,string vvalue,bool const_,int depth_ = 0);
    /*
    *Summary:设置变量的值
    *Parameters:
    *   v:变量的值
...
string constVar = "$%Const^%",externVar = "$%Extern^%",ifselVar = "$system$selectIF^%~";
int maxDepth = 0;
bool nosee = false;
...
*Return :处理值
*/
string code(const string & f,int line,string blockOpition,PTIfStates ifStates = NULL);
/*
